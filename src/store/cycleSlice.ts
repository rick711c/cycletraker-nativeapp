...
const cycleSlice = createSlice({
  ...
  reducers: {
    ...
    updateLastCycle(
      state,
      action: PayloadAction<{ startDate: string; endDate: string; length: number }>,
    ) {
...
export function getPhaseForDate(settings: UserSettings, dateStr: string): CyclePhase {
  ...
  if (cycles.length >= 3) {
    ...
    if (completed.length > 0) {
      avgPeriodLength = Math.round(
        completed.reduce((sum, c) => sum + (c.length || 0), 0) / completed.length,
      );
    }
  }
